...
if defined EXEC_LOG (
    echo [%DATE% %TIME%] [INFO] Запуск скрипта сбора информации. Режим: %COLLECTION_MODE% >> "%EXEC_LOG%"
    echo [%DATE% %TIME%] [INFO] Целевая папка создана: %COMPUTER_PATH% >> "%EXEC_LOG%"
)
...
if /i "%COLLECTION_MODE%"=="full" (
    ...
    if defined EXEC_LOG (
        echo [%DATE% %TIME%] [INFO] Созданы папки для логов и ошибок развертывания. >> "%EXEC_LOG%"
    )
)
...
if defined EXEC_LOG (
    echo [%DATE% %TIME%] [INFO] [ШАГ 1] Запуск dxdiag... >> "%EXEC_LOG%"
)
...
if exist "%COMPUTER_PATH%\%COMPUTER_NAME%_diag.txt" (
    if defined EXEC_LOG (
        echo [%DATE% %TIME%] [INFO] DxDiag успешно завершен. >> "%EXEC_LOG%"
    )
) else (
    if defined EXEC_LOG (
        echo [%DATE% %TIME%] [ERROR] Файл диагностики не создан! Возможна ошибка прав или dxdiag. >> "%EXEC_LOG%"
    )
)
...
if defined EXEC_LOG (
    echo [%DATE% %TIME%] [INFO] [ШАГ 2] Поиск установленных версий 1С... >> "%EXEC_LOG%"
)
...
if exist "C:\Program Files\1C\1CE\components" (
    ...
    if defined EXEC_LOG (
        echo [%DATE% %TIME%] [INFO] Папка компонентов найдена и просканирована. >> "%EXEC_LOG%"
    )
) else (
    echo Папка не найдена: C:\Program Files\1C\1CE\components >> "%COMPONENTS_FILE%"
    :: [ДОБАВЛЕНО] Лог ошибки
    if defined EXEC_LOG (
        echo [%DATE% %TIME%] [WARNING] Папка компонентов 1С не найдена по стандартному пути. >> "%EXEC_LOG%"
    )
)
...
if defined EXEC_LOG (
    echo [%DATE% %TIME%] [INFO] [ШАГ 3] Начат поиск логов 1С... >> "%EXEC_LOG%"
)
...
if exist "%LOGS_SOURCE%" (
    ...
    if defined LATEST_FOLDER (
        ...
        if defined EXEC_LOG (
            echo [%DATE% %TIME%] [INFO] Обнаружена свежая папка логов: !LATEST_FOLDER_NAME!. Копируем... >> "%EXEC_LOG%"
        )
        ...
        if errorlevel 1 (
            ...
            if defined EXEC_LOG (
                echo [%DATE% %TIME%] [ERROR] Ошибка xcopy при копировании логов из !LATEST_FOLDER!. >> "%EXEC_LOG%"
            )
        ) else (
            ...
            if defined EXEC_LOG (
                echo [%DATE% %TIME%] [INFO] Логи успешно скопированы. >> "%EXEC_LOG%"
            )
        )
    ) else (
        ...
        if defined EXEC_LOG (
            echo [%DATE% %TIME%] [WARNING] Папка logs пуста или не содержит подпапок. >> "%EXEC_LOG%"
        )
    )
) else (
    ...
    if defined EXEC_LOG (
        echo [%DATE% %TIME%] [WARNING] Исходная папка логов 1C не найдена (%LOGS_SOURCE%). >> "%EXEC_LOG%"
    )
)

echo [4/8] Копирование deployment_errors (папки 1ce-installer из TEMP)...
if defined EXEC_LOG (
    echo [%DATE% %TIME%] [INFO] [ШАГ 4] Поиск ошибок развертывания в TEMP... >> "%EXEC_LOG%"
)
set "TEMP_PATH=%TEMP%"
...
if exist "%TEMP_PATH%\1ce-installer-crash" (
    ...
    if defined EXEC_LOG (
        echo [%DATE% %TIME%] [INFO] Обнаружен краш-дамп инсталлятора. >> "%EXEC_LOG%"
    )
...
if defined LATEST_INSTALLER_FOLDER (
    ...
    if errorlevel 1 (
        ...
        if defined EXEC_LOG (
            echo [%DATE% %TIME%] [ERROR] Ошибка копирования папки инсталлятора. >> "%EXEC_LOG%"
        )
    ) else (
        echo    - Папка !LATEST_INSTALLER_NAME! успешно скопирована
        if defined EXEC_LOG (
            echo [%DATE% %TIME%] [INFO] Папка инсталлятора скопирована. >> "%EXEC_LOG%"
        )
    )
) else (
...
if defined EXEC_LOG (
    echo [%DATE% %TIME%] [INFO] [ШАГ 5] Сбор списка процессов... >> "%EXEC_LOG%"
)
...
if %errorlevel% neq 0 (
    if defined EXEC_LOG (
        echo [%DATE% %TIME%] [ERROR] Не удалось выполнить tasklist. >> "%EXEC_LOG%"
    )
)
...
if defined EXEC_LOG (
    echo [%DATE% %TIME%] [INFO] [ШАГ 6] Обработка рабочих пространств... >> "%EXEC_LOG%"
)
...
if not exist "%INPUT_FILE%" (
    ...
    if defined EXEC_LOG (
        echo [%DATE% %TIME%] [WARNING] Конфигурационный файл workspace не найден. >> "%EXEC_LOG%"
    )
    goto :SkipWorkspaces
)
...
for /f "tokens=1* delims=," %%a in ("!JSON_CONTENT!") do (
    ...
    if exist "!WIN_PATH!\*" (
        ...
        if errorlevel 1 (
            ...
            if defined EXEC_LOG (
                echo [%DATE% %TIME%] [ERROR] Ошибка копирования папки workspace: !WIN_PATH! >> "%EXEC_LOG%"
            )
        ) else (
        ...
        if errorlevel 1 (
            ...
            if defined EXEC_LOG (
                echo [%DATE% %TIME%] [ERROR] Ошибка копирования файла workspace: !WIN_PATH! >> "%EXEC_LOG%"
            )
        ) else (
        ...
        if defined EXEC_LOG (
            echo [%DATE% %TIME%] [WARNING] Путь workspace не найден на диске: !WIN_PATH! >> "%EXEC_LOG%"
        )
    )
